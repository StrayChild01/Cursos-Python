# Temas varios en Python
### Este repositorio tiene algunos cursos de O'Reilly sobre Python.

- [Getting started with Pandas and Mastering pandas](Pandas/README.md)
  - [Explore, Visualize, and Predict using Pandas & Jupyter](Pandas/01_introduction_get_inspect_tweak_data.ipynb)
  - [Basic Stats & Filtering](Pandas/02_basic_stats_filtering_nans.ipynb)
  - [Grouping & Pivoting](Pandas/03_grouping_pivoting.ipynb)
  - [Grouping & Pivoting 2](Pandas/05_grouping_pivoting_revisited.ipynb)
  - [Machine Learning(?)](Pandas/04_machine_learning.ipynb)  
- [Hands-on Machine Learning with Python: Classification and Regression](ML+Python_Classification/README.md)
  - [Regression](ML+Python_Classification/Class/02-Regression.ipynb)
  - [Classification](ML+Python_Classification/Class/03-Classification.ipynb)
- [Hands-on Machine Learning with Python: Clustering, Dimension Reduction, and Time Series Analysis](ML+Python_Clustering/README.md)
  - [PCA](ML+Python_Clustering/Class/02-PCA.ipynb)
  - [Clustering](ML+Python_Clustering/Class/Clustering.ipynb)
  - [Time Series](ML+Python_Clustering/Class/04-TimeSeries.ipynb)
